...
class Client {
  ...
  int? getMyId(){
    ...
    ).key==-1?null:serverSendDto.playersWithId.entries.firstWhere((e)=>e.value==clientName).key;
  }
  ...
  Future<void> _discoverAndConnect()async{
    try{
      ...
      tcpSocket=await Socket.connect(host.$1, host.$2,timeout: const Duration(seconds: 5));
      _send(clientSendDto);
      tcpSub=tcpSocket.lines.listen((line){
